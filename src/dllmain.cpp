...
void Framerate()
{
    ...
    if (bAdjustClothPhysics)
    {
        ...
        // Cloth physics
        std::uint8_t* ClothPhysicsScanResult = Memory::PatternScan(exeModule, "4C 8B ?? ?? 49 8B ?? ?? 45 0F ?? ?? ?? ?? ?? ?? 45 0F ?? ?? ?? ?? ?? ?? 41 ?? ?? 07");
...
void Misc()
{
    if (bDisablePhotoModePillarboxing) 
    {
        ...
        std::uint8_t* PhotoModePillarboxingScanResult = Memory::PatternScan(exeModule, "C4 ?? ?? ?? ?? 89 ?? ?? 0F 83 ?? ?? ?? ?? E9 ?? ?? ?? ?? 83 ?? ?? ?? 01 0F ?? ?? 41 ?? ?? 01");
